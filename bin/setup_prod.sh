...
oc set probe dc/tasks-green --readiness --get-url=http://:8080/ --initial-delay-seconds=40 --timeout-seconds=1 -n ${GUID}-tasks-prod
oc set probe dc/tasks-green --liveness --get-url=http://:8080/ --initial-delay-seconds=40 --timeout-seconds=1 -n ${GUID}-tasks-prod
# Setting 'wrong' VERSION. This will need to be updated in the pipeline
